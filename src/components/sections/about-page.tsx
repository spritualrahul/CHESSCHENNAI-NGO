...
import { ScrollReveal, StaggerContainer, StaggerItem, RevealSection } from "@/components/shared/scroll-reveal";
import {
  aboutHero,
  aboutStats,
  boardMembers,
  ...
  return (
    <section className="relative isolate min-h-[470px] overflow-hidden bg-[var(--ches-blue)] pt-20 text-white">
      ...
      <div className="relative mx-auto flex min-h-[470px] max-w-6xl items-center px-5 py-20">
        ...
        >
          <motion.h1
            className="font-heading text-6xl font-medium leading-none md:text-7xl"
          ...
          <motion.p
            className="mt-6 text-base leading-7 text-white/86 md:text-lg"
          ...
          <motion.div
            className="mt-5 flex items-center gap-3 border-t border-[#d9a13b]/80 pt-4 text-sm font-semibold text-white/90"
...
function AboutStats() {
  return (
    <StaggerContainer
      className="relative z-10 mx-auto -mt-8 grid max-w-6xl grid-cols-2 overflow-hidden rounded-xl border border-[#e1e9e7] bg-white shadow-[0_8px_28px_rgb(11_78_109/0.12)] md:grid-cols-4"
      stagger={stagger.medium}
      margin="-5% 0px"
    >
      {aboutStats.map((stat) => (
        <StaggerItem key={stat.title} variant="fadeUpSmall">
          <div className="flex min-h-[86px] items-center gap-3 border-[#e1e9e7] px-4 py-4 md:px-6 md:[&:not(:first-child)]:border-l">
...
function ProgressSection() {
  return (
    <RevealSection className="bg-white px-5">
      <div className="mx-auto grid max-w-6xl gap-10 lg:grid-cols-[.8fr_2.2fr] lg:items-center lg:gap-14">
        ...
        <div className="relative">
          <StaggerContainer className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4" stagger={stagger.medium}>
            {progressCards.map((card) => (
              <StaggerItem key={card.period} as="article" variant="fadeUpSmall">
                <div className="overflow-hidden rounded-xl border border-[#e1e7e5] bg-white shadow-[0_8px_20px_rgb(11_78_109/0.05)] transition hover:-translate-y-1">
  ...
  return (
    <RevealSection className="bg-[var(--ches-sky)] px-5">
      <div className="mx-auto grid max-w-6xl gap-10 lg:grid-cols-[.9fr_1.1fr] lg:items-center lg:gap-14">
        ...
        <div>
          <ScrollReveal variant="fadeUpSmall">
          ...
          <StaggerContainer className="mt-5 space-y-4 text-sm leading-6 text-[var(--ches-muted)]" stagger={stagger.medium}>
            {journeyIntro.paragraphs.map((paragraph) => <StaggerItem key={paragraph}><p>{paragraph}</p></StaggerItem>)}
            <StaggerItem><p>CHES has consistently evolved to address emerging needs — from HIV prevention and care to child protection, education, gender equity and community well-being.</p></StaggerItem>
          </StaggerContainer>
          <StaggerContainer className="mt-7 grid grid-cols-2 gap-4 border-t border-[#cbdde5] pt-6 sm:grid-cols-4" stagger={stagger.fast}>
            {[
              ["30+", "Years of Service", 30],
              ["310+", "Children Cared For", 310],
              ["1M+", "People Reached", null],
              ["Many", "Lives Transformed", null],
            ].map(([value, label, numericValue]) => (
              <StaggerItem key={label as string} variant="fadeUpSmall">
                <div>
                  <p className="font-heading text-2xl font-semibold text-[var(--ches-blue)]">
                    {typeof numericValue === "number" ? (
                      <Counter value={numericValue as number} suffix="+" duration={2} />
                    ) : (
                      value as string
                    )}
                  </p>
                  <p className="mt-1 text-[0.66rem] leading-4 text-[var(--ches-muted)]">{label as string}</p>
                </div>
              </StaggerItem>
            ))}
          </StaggerContainer>
...
function JourneyThumbnail({ src, alt }: { src: string; alt: string }) {
  return (
    <ScrollReveal variant="scaleReveal" delay={0.1}>
      <div className="relative aspect-square overflow-hidden rounded-lg">
        <Image src={src} alt={alt} fill sizes="(min-width: 1024px) 14vw, 30vw" className="object-cover" />
      </div>
    </ScrollReveal>
  );
}
...
function ImpactAreasSection() {
  return (
    <RevealSection id="impact" className="bg-[#fbfcfb] px-5">
      <div className="mx-auto max-w-6xl">
        <ScrollReveal variant="fadeUpSmall" className="text-center">
          <p className="text-xs font-bold uppercase tracking-[0.16em] text-[var(--ches-orange)]">Our Impact Areas</p>
        </ScrollReveal>
        <StaggerContainer className="mt-7 grid gap-4 sm:grid-cols-2 lg:grid-cols-5" stagger={stagger.fast}>
          {impactAreas.map((area) => (
            <StaggerItem key={area.title} as="article" variant="fadeUpSmall">
              <div className="rounded-xl border border-[#e1e7e5] bg-white p-5 shadow-[0_8px_24px_rgb(11_78_109/0.04)] transition hover:-translate-y-1">
...
function FullJourneySection() {
  return (
    <RevealSection id="journey" className="bg-[var(--ches-sky)] px-5">
      <div className="mx-auto max-w-6xl">
        <ScrollReveal variant="fadeUpSmall">
        ...
        <StaggerContainer className="mt-10 grid gap-x-10 gap-y-8 md:grid-cols-2" stagger={stagger.medium}>
          {journeySections.map((section, index) => (
            <StaggerItem key={section.title} as="article" variant="fadeUpSmall">
              <div className="border-t border-[#cbdde5] pt-5">
      ...
      <div className="mx-auto max-w-6xl">
        <div className="grid gap-10 lg:grid-cols-[.65fr_1.35fr] lg:items-start lg:gap-14">
          ...
          <div>
            ...
            <StaggerContainer className="mt-7 space-y-5 text-sm leading-7 text-[var(--ches-muted)]" stagger={stagger.fast}>
              {founderParagraphs.map((paragraph) => <StaggerItem key={paragraph}><p>{paragraph}</p></StaggerItem>)}
            </StaggerContainer>
            <ScrollReveal variant="fadeUpSmall" delay={0.2}>
      ...
      <div className="mx-auto max-w-6xl">
        <div className="grid gap-12 lg:grid-cols-[.85fr_1.15fr] lg:items-start">
          ...
          <div>
            ...
            <StaggerContainer className="mt-7 grid gap-4 sm:grid-cols-2" stagger={stagger.medium}>
              {managementTeam.map((member) => (
                <StaggerItem key={member.name} as="article" variant="scaleReveal">
                  <div className="overflow-hidden rounded-xl border border-[#dce7e5] bg-white shadow-[0_8px_24px_rgb(11_78_109/0.05)]">
                    <div className="relative aspect-[1.25] bg-[#eef4f3]"><Image src={member.image} alt={`${member.name}, ${member.role}`} fill sizes="(min-width: 1024px) 25vw, 80vw" className="object-cover" /></div>
                    <div className="p-4"><h3 className="text-sm font-bold text-[var(--ches-ink)]">{member.name}</h3><p className="mt-1 text-xs font-semibold text-[var(--ches-orange)]">{member.role}</p><p className="mt-3 text-xs leading-5 text-[var(--ches-muted)]">{member.text}</p></div>
                  </div>
                </StaggerItem>
              ))}
            </StaggerContainer>
